...
"""
12/4/23 Trying to take into accout the time delay of the motor to reach a desired torque and match
the physical system as best as possible.
"""
...
def simulate_inverted_pendulum(initial_theta_degrees, simulation_duration=10):
    ...
    global previous_torque, tau

    # Initialize the motor time constant (tau)
    # You may need to adjust this value based on your motor's characteristics
    tau = 0.5  # Example value, in seconds

    # Initialize the previous torque
    # Assuming the motor starts with no initial torque
    previous_torque = 0
    ...
    def equations_of_motion_with_controller(t, y):
        ...
        desired_torque = -K.dot(x)[0, 0]

        # First-order lag for the time delay
        # tau is the time constant of the motor
        actual_torque = (1 - np.exp(-t / tau)) * desired_torque + np.exp(-t / tau) * previous_torque
        previous_torque = actual_torque

        d_theta_dot = (m_p * g * l * np.sin(theta) - actual_torque) / (I_p + m_p * l**2)
        d_phi_dot = actual_torque / I_w
...
# Example usage:
anim, fig = simulate_inverted_pendulum(50)
